...
var UserFilters = Widget.extend({
	...
	start : function(){
        ...
        var company_filter = []
        if (self.session.user_companies){
            var i;
            for(i = 0; i < self.session.user_companies.allowed_companies.length; i++) {
                allowed_companies.push(parseInt(self.session.user_companies.allowed_companies[i][0]));
            }
            company_filter = [['id','in',allowed_companies]];
            }
        else{
            company_filter = [['id','=',current_company]];
        }
