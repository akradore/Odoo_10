...
class field_ids_one2many(models.Model):
    ...
    def create(self,vals):
        res = super(field_ids_one2many,self).create(vals)
        if res.field_id.ttype == 'binary' or res.field_id.ttype == 'html':
            raise UserError(_('You can not select HTML or Binary Fields!!'))
        else:
            return res
        return res
    ...
    def onchnage_field_id(self):
        for field in self:
            if field.field_id:
                field.label = field.field_id.field_description
        return
...
class sub_model_field_ids_one2many(models.Model):
    ...
    @api.model
    def create(self,vals):
        res = super(sub_model_field_ids_one2many,self).create(vals)
        if res.field_id.ttype == 'binary' or res.field_id.ttype == 'html':
            raise UserError(_('You can not select HTML or Binary Fields!!'))
        else:
            return res
...
class generic_excel_report_wizard(models.TransientModel):
            ...
            if count>=1 or globle_excel_report_id.sheet_per_page:
                for field in globle_excel_report_id.field_ids:
                    ...
                    if field.field_id.ttype=="datetime":
                        ...
                        record = datetime.strftime(pytz.utc.localize(datetime.strptime(data, DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local_tz),"%d/%m/%Y %H:%M:%S") 
    ...
    def detailed_excel_report(self):
        ...
        for record in self._context.get('active_ids'):
            browse_record = self.env[self._context.get('active_model')].browse(record)
            if globle_excel_report_id.sheet_per_page:
                sheet_name = str(globle_excel_report_id.template_id.excel_sheet_name)+ str(record)
                worksheet = workbook.add_sheet(sheet_name)
                self._write_header(worksheet)
                row= self._write_company_detail(row=3,col=0,worksheet=worksheet)
            col = 0
            ...
            for header_value in globle_excel_report_id.field_ids:
                ...
                if header_value.field_id.ttype=="datetime":
                        ...
                        record_data = datetime.strftime(pytz.utc.localize(datetime.strptime(data, DEFAULT_SERVER_DATETIME_FORMAT)).astimezone(local_tz),"%d/%m/%Y %H:%M:%S") 
